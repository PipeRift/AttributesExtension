...
public:
	...
	static FORCEINLINE bool Is(const FFloatAttr& A, const FFloatAttr& B)
	{
		return A == B;
	}
	...
	UFUNCTION(BlueprintPure, Category = Attributes, meta = (CompactNodeTitle = "!="))
	static FORCEINLINE bool IsNot(const FFloatAttr& A, const FFloatAttr& B)
	{
		return A != B;
	}
	...
	static FORCEINLINE float GetValue(const FFloatAttr& Attribute)
	{
	...
	// Get final value
	UFUNCTION(BlueprintPure, Category = Attributes,
		meta = (DisplayName = "ToFloat (FloatAttr)", CompactNodeTitle = "->", Keywords = "get value float",
			BlueprintAutocast))
	static FORCEINLINE float Conv_AttributeToFloat(const FFloatAttr& Attribute)
	{
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = Attributes)
	static FORCEINLINE float GetBase(const FFloatAttr& Attribute)
	{
	...
	static void SetBase(UPARAM(ref) FFloatAttr& Attribute, float Value)
	{
	...
	static void AddModifier(
		UPARAM(ref) FFloatAttr& Attribute, const FAttrModifier& Modifier, const FAttrCategory Category)
	{
	...
	static FORCEINLINE bool RemoveModifier(UPARAM(ref) FFloatAttr& Attribute, const FAttrModifier& Modifier,
	...
	static void GetModifiers(
		const FFloatAttr& Attribute, const FAttrCategory Category, TArray<FAttrModifier>& Modifiers)
	{
...
protected:
	UFUNCTION(BlueprintPure, Category = Attributes)
	static void Make(float BaseValue, FFloatAttr& FloatAttr)
	{
		FloatAttr = {BaseValue};
	}
